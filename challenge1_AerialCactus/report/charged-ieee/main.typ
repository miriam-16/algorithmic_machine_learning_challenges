...
#show: ieee.with(
  ...
  abstract: [
    This report presents a comparative analysis of different machine learning models developed to classify aerial images containing a specific type of cactus (Neobuxbaumia tetetzo). The goal of the project is to support biodiversity monitoring efforts, such as the VIGIA project in Mexico, by identifying the presence of cacti in 32×32 aerial image patches. We describe the dataset, class imbalance issues, preprocessing techniques (including targeted data augmentation), and the models developed: logistic regression, support vector machine (SVM), a custom convolutional neural network (CNN), and a transfer learning approach using ResNet18. Each model was evaluated using accuracy and F1 score, with ResNet18 achieving the best overall performance. Based on a balance of generalization and robustness, ResNet18 was selected for final predictions on the unlabeled test set. To further enhance reliability, a weighted ensemble of all models was used to generate the final output. This work confirms the value of deep learning and transfer learning in ecological monitoring through aerial imagery.
  ],
  authors: (
    (
      name: "Alex Argese",
...
The preprocessing involved first converting the images from *PIL* to *PyTorch tensors* and resizing them to a fixed resolution of *32×32 pixels*, consistent with the format of the original images.

== Data augmentation  
To address the obvious *class imbalance* (about *13000* images for class 1 and only *4000* for class 0), a *data augmentation strategy* was applied targeting only *minority class images*. Small *random rotations (±10°)* were applied, and then resized to *32×32 pixels*, generating about *4000 new synthetic images* of class 0, bringing the total to ~8000 images for this class. This has significantly *reduced the difference* between the two classes, improving the *ability of the models to generalize*.
...
#figure(
  image("img/distribution_after.png", width: 85%),
  caption: [
